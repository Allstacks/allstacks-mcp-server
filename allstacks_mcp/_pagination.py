...
def cap_list_payload(payload: Any, *, limit: int, offset: int = 0) -> Any:
    """Slice list payloads when the API returns rows outside the requested page."""
    if isinstance(payload, list) and (offset > 0 or len(payload) > limit):
        return payload[offset : offset + limit]

    if isinstance(payload, dict):
        results = payload.get("results")
        if isinstance(results, list) and (offset > 0 or len(results) > limit):
            capped = dict(payload)
