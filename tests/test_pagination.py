...
class CapListPayloadTests(unittest.TestCase):
    ...
    def test_applies_offset_on_short_top_level_list(self):
        payload = [{"id": i} for i in range(8)]
        capped = cap_list_payload(payload, limit=10, offset=5)
        self.assertEqual([row["id"] for row in capped], [5, 6, 7])
    ...
    def test_applies_offset_on_short_results_wrapper(self):
        payload = {"count": 30, "results": [{"id": i} for i in range(8)]}
        capped = cap_list_payload(payload, limit=10, offset=5)
        self.assertEqual([row["id"] for row in capped["results"]], [5, 6, 7])
        self.assertEqual(capped["count"], 30)
